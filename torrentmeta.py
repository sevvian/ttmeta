...
class TorrentParser:
    def __init__(self):
        ...
        self.tld_pattern = self.get_tld_pattern()

    def get_tld_pattern(self):
        """Fetch the latest TLD list from IANA and create a regex pattern"""
        try:
            IANA_TLD_URL = "https://data.iana.org/TLD/tlds-alpha-by-domain.txt"
            response = requests.get(IANA_TLD_URL, timeout=10)
            lines = response.text.splitlines()

            # The first line is a version comment; the rest are valid TLDs
            tlds = set(line.lower() for line in lines if not line.startswith("#") and line.strip())

            # Add uncommon TLDs used by torrent sites (small exception list)
            uncommon_torrent_tlds = {'za', 'mu', 'by', 'tax', 'ws'}
            tlds.update(uncommon_torrent_tlds)

            # Build regex pattern
            escaped = [re.escape(tld) for tld in sorted(tlds)]
            pattern = r"\.(" + "|".join(escaped) + r")(?![A-Za-z])"
            return re.compile(pattern, re.IGNORECASE)

        except Exception:
            # Return None if IANA fetch fails
            return None
...
#        ]
    ...
    def _compile_filesize_patterns(self) -> List[Tuple[str, re.Pattern]]:
        return [
            # Decimal patterns first (more specific)
            ("###.#GB", re.compile(r'\b(\d+\.\d+)GB\b', re.IGNORECASE)),
            ("###.#MB", re.compile(r'\b(\d+\.\d+)MB\b', re.IGNORECASE)),
            ("###.#TB", re.compile(r'\b(\d+\.\d+)TB\b', re.IGNORECASE)),
            # Whole number patterns second
            ("###GB", re.compile(r'\b(\d+)GB\b', re.IGNORECASE)),
            ("###MB", re.compile(r'\b(\d+)MB\b', re.IGNORECASE)),
            ("###TB", re.compile(r'\b(\d+)TB\b', re.IGNORECASE)),
            ("###KB", re.compile(r'\b(\d+)KB\b', re.IGNORECASE)),
    ...
    def extract_title(self, torrent_name):
        """
        Extracts the title from a messy torrent name with enhanced rules:
        1. If string starts with brackets, remove that bracket content first
        2. Break into tokens when ANY bracket starts (first occurrence)
        3. Removing tokens that start with brackets
        4. Strict website pattern detection using TLDs (excluding exception TLDs)
        """

        # Get TLD pattern
        tld_pattern = self.get_tld_pattern()

        # Enhancement 1: Remove bracket content if string starts with brackets
        cleaned_name = torrent_name.strip()
        if cleaned_name and cleaned_name[0] in '([{':
            # Find the matching closing bracket
            opening_bracket = cleaned_name[0]
            ...
            if end_pos > 0:
                cleaned_name = cleaned_name[end_pos:].strip()

        # Step 1: Break into tokens when any bracket starts (first occurrence only)
        tokens = []
        current_token = ""
        bracket_found = False

        for char in cleaned_name:
            if char in '([{' and not bracket_found:
                # First bracket found - split here
        ...
        # Step 2: Remove tokens that start with brackets
        filtered_tokens = []
        for token in tokens:
            if not token.startswith(('(', '[', '{')):
                filtered_tokens.append(token)

        # Step 3: Clean up the remaining text with strict website detection
        if filtered_tokens:
            potential_title = ' '.join(filtered_tokens)

            # Remove website patterns only if we have valid TLD pattern
            if tld_pattern:
                # Define exception TLDs
                exception_tlds = {'za', }

                # Create a custom replacement function that checks for exception TLDs
                def website_replacer(match):
                    matched_text = match.group(0)
                    # Check if the matched text ends with an exception TLD
                    for tld in exception_tlds:
                        if matched_text.lower().endswith('.' + tld):
                            return matched_text  # Keep it if it's an exception TLD
                    return ''  # Remove it if it's not an exception TLD

                # Correct website patterns - these should match domain.tld format
                website_patterns = [
                    r'\bwww\.\w+' + tld_pattern.pattern,   # www.something.tld
                    r'\b\w+' + tld_pattern.pattern,        # something.tld
                    r'-\s*\w+' + tld_pattern.pattern,      # - something.tld
                    r'@\s*\w+' + tld_pattern.pattern,      # @ something.tld
                ]

                for pattern_str in website_patterns:
                    pattern = re.compile(pattern_str, re.IGNORECASE)
                    potential_title = pattern.sub(website_replacer, potential_title)

            # Clean up any extra spaces, dashes, or punctuation
            potential_title = re.sub(r'\s+', ' ', potential_title)
            potential_title = re.sub(r'^\s*[-.\s]+\s*|\s*[-.\s]+\s*$', '', potential_title)
    ...
    def post_process_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "season" in result:
            ...
            if season_value:
                processed["season"] = self._process_season_episode_field(season_value)
            ...
            if episode_value:
                ...
                if ranges:
                    ...
                    if best_range and max_span > 0:
                        ...
                        processed["episode"] = self._process_season_episode_field(episode_value)
                else:
                    processed["episode"] = self._process_season_episode_field(episode_value)
    ...
    def _process_season_episode_field(self, field_value):
    ...
    def parse_to_json(self, title: str) -> str:
        """Parse single title and return JSON string"""
        result = self.parse(title)
        processed_result = self.post_process_result(result)
        return json.dumps(processed_result, indent=2, ensure_ascii=False)

    def parse_multiple_to_json(self, titles: List[str]) -> str:
        """Parse multiple titles and return JSON string"""
        results = self.parse_multiple(titles)
        return json.dumps(results, indent=2, ensure_ascii=False)

    def parse_multiple(self, titles: List[str]) -> List[Dict[str, Any]]:
        """Parse multiple titles and return list of JSON results"""
        results = []
        for title in titles:
            result = self.parse(title)
            processed_result = self.post_process_result(result)
...
if __name__ == "__main__":
    ...
    # Parse single title
    single_result = parser.parse_to_json(test_titles[0])
    print(single_result)

    # Parse multiple titles
    multiple_results = parser.parse_multiple_to_json(test_titles)
    print(multiple_results)

    # Or get as Python objects instead of JSON strings
    single_dict = parser.parse(test_titles[0])
    multiple_dicts = parser.parse_multiple(test_titles)
